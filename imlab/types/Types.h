...
// ---------------------------------------------------------------------------
#ifndef IMLAB_TYPES_TYPES_H_
#define IMLAB_TYPES_TYPES_H_
//---------------------------------------------------------------------------
...
struct LengthIndicator {
    typedef typename LengthSwitch<((kMaxLen < 256) ? 1 : (kMaxLen < 65536) ? 2 :
                                                                             4)>::type type;
};
...
class Numeric {
    ...
    static Numeric<len, precision> castString(const char* str, uint32_t strLen) {
        ...
        // Parse
        if (iter == limit) {
            throw RuntimeException("invalid number format: found non-numeric characters");
        }
        ...
        for (; iter != limit; ++iter) {
            ...
            if ((c >= '0') && (c <= '9')) {
                ...
                if (fraction) {
                    throw RuntimeException("invalid number format: already in fraction");
                }
                while ((iter != limit) && ((*(limit - 1)) == '0')) --limit; // skip trailing 0s
                fraction = true;
            } else {
                throw RuntimeException("invalid number format: invalid character in numeric string");
            }
        }

        if ((digitsSeen > 18 /*(len-precision)*/) || (digitsSeenFraction > precision)) {
            throw RuntimeException("invalid number format: loosing precision");
        }
...
class Value {
    ...
    Value() : type_(Type::Integer()), data_(Integer(0)) {}

    explicit Value(const Integer& value)
        : type_(Type::Integer()), data_(value) {}

    explicit Value(int64_t numericRawValue, const Type& numericType)
        : type_(numericType), data_(numericRawValue) {}

    explicit Value(const std::string& strValue, const Type& stringType)
        : type_(stringType), data_(strValue) {}

    explicit Value(const Timestamp& timestamp)
        : type_(Type::Timestamp()), data_(timestamp) {}

    Integer getInteger() const {
        return std::get<Integer>(data_);
    }
    int64_t getNumericRawValue() const {
    ...
    static std::string getTypeName(const Type& type) {
        switch (type.getClass()) {
            case Type::kInteger:
                return "imlab::Integer";
            case Type::kTimestamp:
                return "imlab::Timestamp";
            case Type::kNumeric:
                // Assuming Numeric has template arguments len and precision
                    return "imlab::Numeric<" + std::to_string(type.getPrecision()) + ", " + std::to_string(type.getScale()) + ">";
            case Type::kChar:
                return "imlab::Char<" + std::to_string(type.getLength()) + ">";
            case Type::kVarchar:
                return "imlab::Varchar<" + std::to_string(type.getLength()) + ">";
            default:
                throw std::runtime_error("Unknown type class");
        }
    }
    ...
    uint64_t hash() const {
        switch (type_.getClass()) {
            case Type::kInteger:
                return std::get<Integer>(data_).hash();
            case Type::kTimestamp:
                return std::get<Timestamp>(data_).hash();
            case Type::kNumeric:
            ...
            default:
                throw std::runtime_error("Unsupported type for hashing");
        }
    }
...
inline bool operator==(const Value& lhs, const Value& rhs) {
    ...
    switch (lhs.getType().getClass()) {
        case Type::kInteger:
            return lhs.getInteger() == rhs.getInteger();
        case Type::kTimestamp:
            return lhs.getTimestamp() == rhs.getTimestamp();
        case Type::kNumeric:
            return lhs.getNumericRawValue() == rhs.getNumericRawValue();
        case Type::kVarchar:
        case Type::kChar:
            return lhs.getStringValue() == rhs.getStringValue();
        default:
            throw std::runtime_error("Unsupported type in operator==");
    }
}
...
namespace std {
template <>
struct hash<imlab::Value> {
    size_t operator()(const imlab::Value& value) const noexcept {
        return value.hash();
    }
};
}
...
namespace std {
template <>
struct hash<imlab::Integer> {
    size_t operator()(const imlab::Integer &integer) const noexcept {
        return integer.hash();
    }
};
}
//---------------------------------------------------------------------------
#endif // IMLAB_TYPES_TYPES_H_
