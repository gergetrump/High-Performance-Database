...
Integer Integer::castString(const char* str, uint32_t strLen) {
    ...
    // Parse
    if (iter == limit) {
        throw RuntimeException("invalid number format: found non-integer characters");
    }
    ...
    for (; iter != limit; ++iter) {
        ...
        if ((c >= '0') && (c <= '9')) {
            ...
            throw RuntimeException("invalid number format: invalid character in integer string: '" + std::string(str, strLen) + "'");
        }
    }

    if (digitsSeen > 10) {
        throw RuntimeException("invalid number format: too many characters (32bit integers can at most consist of 10 numeric characters)");
    }
...
//---------------------------------------------------------------------------
static const uint64_t msPerDay = 24ull * 60 * 60 * 1000;
//---------------------------------------------------------------------------
...
Integer extractYear(const Date& d) {
    ...
    Integer r(static_cast<int32_t>(year));
    return r;
}
//---------------------------------------------------------------------------
...
Date Date::castString(const char* str, uint32_t strLen) {
    ...
    unsigned year = 0;
    while (true) {
        if (iter == limit) throw RuntimeException("invalid date format");
        char c = *(iter++);
        if (c == '-') break;
        if ((c >= '0') && (c <= '9')) {
            year = 10 * year + (c - '0');
    ...
    Date d;
    d.value = static_cast<int32_t>(mergeJulianDay(year, month, day));
    return d;
}
//---------------------------------------------------------------------------
...
Timestamp Timestamp::castString(const char* str, uint32_t strLen) {
    ...
    unsigned year = 0;
    while (true) {
        if (iter == limit) throw RuntimeException("invalid timestamp format");
        char c = *(iter++);
        if (c == '-') break;
        if ((c >= '0') && (c <= '9')) {
            year = 10 * year + (c - '0');
        } else {
            throw RuntimeException("invalid timestamp format");
        }
    }
    // Month
    unsigned month = 0;
